...
/**
 * Class Nice_Infoboxes_MetaboxCreateResponder
 *
 * This class takes charge of the interaction of created Nice_Infoboxes_Metabox
 * instances with WordPress APIs.
 *
 * @since 1.1
 */
class Nice_Infoboxes_MetaboxCreateResponder extends Nice_Infoboxes_CreateResponder {
	/**
	...
	protected function get_metabox_settings( $post_type ) {
		...
		$defaults = array(
			'id' 			=> $data->id ? : 'nice-infoboxes-' . $post_type . '-settings',
			'title' 		=> sprintf( esc_html__( '%s Settings', 'nice-infoboxes' ), ucfirst( $post_type ) ),
			'callback' 		=> 'nice_infoboxes_metabox_print',
			'post_type'     => $post_type,
		...
		$settings = apply_filters( 'nice_infoboxes_metabox_' . $post_type . '_settings', $settings );
