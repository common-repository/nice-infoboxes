...
/**
 * Class Nice_Infoboxes_Metabox_HTMLHandler
 *
...
class Nice_Infoboxes_Metabox_HTMLHandler {
	 ...
	 * @since 1.1
	 *
	 * @param Nice_Infoboxes_Metabox $instance
	 */
	public function __construct( Nice_Infoboxes_Metabox $instance ) {
	...
	protected function process_html() {
		...
		foreach ( $this->data->fields as $field ) {
			...
			$field = apply_filters( 'nice_infoboxes_post_type_metabox_add_field', $field, $this );
			// Apply a type-specific filter.
			$field = apply_filters( 'nice_infoboxes_post_type_metabox_add_field_' . $field['type'], $field, $this );
			...
			// Apply a generic filter.
			$field_html = apply_filters( 'nice_infoboxes_post_type_metabox_add_field_html', $field_html, $field, $this );
			// Apply a type-specific filter.
			$field_html = apply_filters( 'nice_infoboxes_post_type_metabox_add_field_html_' . $field['type'], $field_html, $field, $this );
	...
	protected static function custom_get_textarea( $field ) {
		...
		$html .= "\t\t" . '<td><textarea class="nice_infoboxes_textarea " name="' . $field['name'] . '" id="' . $id . '">' . esc_textarea( stripslashes( html_entity_decode( self::custom_get_value( $field ) ) ) ) . '</textarea>';
		$html .= '<span class="description">' . $field['desc'] . '</span></td>' . "\n";
		$html .= "\t" . '</tr>' . "\n";
	...
	protected static function custom_get_file( $field ) {
		printf( '<p>' . esc_html__( '%s method is not implemented yet. Please use another method to display an option for %s', 'nice-infoboxes' ) . '</p>', __METHOD__ . '()', esc_html( $field['name'] ) . '<br />' );
	}
	...
	protected static function custom_get_radio( $field ) {
		...
		if ( $options ) {
			...
			foreach ( $options as $id => $option ) {
				...
				$html .= '<span class="description" style="display:inline">' . $option . '</span><div class="nice_infoboxes_spacer"></div>';
			}
