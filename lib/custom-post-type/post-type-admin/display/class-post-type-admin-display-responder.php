...
/**
 * Class Nice_Infoboxes_Post_Type_AdminDisplayResponder
 *
 * This class takes charge of the Nice_Infoboxes_Post_Type_Admin instances to
 * be displayed through WordPress APIs.
 *
 * @since 1.1
 */
class Nice_Infoboxes_Post_Type_AdminDisplayResponder extends Nice_Infoboxes_DisplayResponder {
	/**
	...
	public function add_image_column( $columns ) {
		...
		if ( $this->data->post_type->{'args'}['name'] === $_typenow ) {
			...
			$image_column = apply_filters(
				'nice_infoboxes_post_type_' . $this->data->post_type->{'args'}['name'] . '_add_image_column',
				$image_column,
				$columns,
				$this
			);
	...
	public function display_image( $column ) {
		...
		if ( $this->data->post_type->{'args'}['name'] === $post_type ) {
			...
			// Apply general filters.
			$image = apply_filters(
				'nice_infoboxes_post_type_display_image',
				$image,
				$column,
				$this
			);
			...
			// Apply post-type-specific filters.
			$image = apply_filters(
				'nice_infoboxes_post_type_' . $this->data->post_type->{'args'}['name'] . '_display_image',
				$image,
				$column,
				$this
			);
	...
	public function add_taxonomy_filters() {
		...
		foreach ( $this->data->post_type->{'taxonomies'} as $taxonomy ) {
			...
			$taxonomy_filter = apply_filters(
				'nice_infoboxes_post_type_taxonomy_filter',
				$taxonomy_filter,
				$typenow,
				$this
			);
			...
			$taxonomy_filter = apply_filters(
				'nice_infoboxes_post_type_' . $taxonomy['name'] . '_taxonomy_filter',
				$taxonomy_filter,
				$typenow,
				$this
			);
	...
	protected function build_taxonomy_filter( $tax_slug ) {
		...
		// Apply general filters.
		$filter = apply_filters( 'nice_infoboxes_post_type_build_taxonomy_filter', $filter, $tax_slug, $this );

		// Apply taxonomy-specific filters.
		$filter = apply_filters(
			'nice_infoboxes_post_type_build_' . $tax_slug . '_taxonomy_filter',
			$filter,
			$tax_slug,
			$this
		);
	...
	protected function get_taxonomy_name_from_slug( $tax_slug ) {
		...
		$taxonomy_name = apply_filters(
			'nice_infoboxes_post_type_taxonomy_name',
			$taxonomy_name,
			$tax_slug,
			$this
		);
		...
		// Apply taxonomy-specific filters.
		$taxonomy_name = apply_filters(
			'nice_infoboxes_post_type_' . $tax_slug . '_taxonomy_name',
			$taxonomy_name,
			$tax_slug,
			$this
		);
	...
	protected function build_term_options( $terms, $current_tax_slug ) {
		...
		$options = apply_filters(
			'nice_infoboxes_post_type_term_options',
			$options,
			$terms,
			$current_tax_slug,
			$this
		...
		// Apply taxonomy-specific filters.
		$options = apply_filters(
			'nice_infoboxes_post_type_' . $current_tax_slug . '_term_options',
			$options,
			$terms,
			$current_tax_slug,
			$this
