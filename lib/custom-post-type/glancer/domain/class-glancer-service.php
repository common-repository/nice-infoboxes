...
/**
 * Class Nice_Infoboxes_GlancerService
 *
 * This class deals with typical operations over Nice_Infoboxes_Glancer instances.
 *
 * @since 1.1
 */
class Nice_Infoboxes_GlancerService extends Nice_Infoboxes_Service {
	/**
	 * Create and return a new Nice_Infoboxes_Glancer instance.
	 *
	 * @since  1.1
	 *
	...
	protected function setup( array $data ) {
		/**
		 * Obtain an instance of Nice_Infoboxes_Pointer_Collection.
		 */
	 ...
	 *
	 * @uses  Nice_Infoboxes_Glancer::update()
	 *
	 * @since 1.1
	 *
	 ...
	 */
	public function add_item( Nice_Infoboxes_Glancer $glancer, $post_types, $statuses = 'publish', $glyph = '' ) {
		/**
		 * If relevant output action hook has already passed, then there's no
		 * point in proceeding.
		 */
		if ( did_action( 'dashboard_glance_items' ) ) {
			_doing_it_wrong( __CLASS__, esc_html__( 'Trying to add At a Glance items to dashboard widget after hook already fired', 'nice-infoboxes' ), '1.1' );
			return;
		}
