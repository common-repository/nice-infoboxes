 ...
 *
 * @uses   Nice_Infoboxes_AutoLoader::__construct()
 *
 * @since  1.0
 *
 ...
 *
 * @return Nice_Infoboxes_AutoLoader
 */
function nice_infoboxes_autoload( $path, $root_directory = '', array $exclude = null ) {
	return new Nice_Infoboxes_AutoLoader( $path, $root_directory, $exclude );
}

/**
 * Load a list of directories recursively.
 *
 * @uses   nice_infoboxes_autoload()
 *
 * @since  1.0
 *
 ...
 * @return Nice_Infoboxes_AutoLoader
 */
function nice_infoboxes_autoload_libraries( array $libraries, $root_directory = '', array $exclude = null ) {
	if ( ! empty( $libraries ) ) {
		foreach ( $libraries as $library ) {
			nice_infoboxes_autoload( $library, $root_directory, $exclude );
		}
	}
}
