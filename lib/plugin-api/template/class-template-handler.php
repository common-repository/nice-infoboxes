...
/**
 * Class Nice_Infoboxes_Template_Handler
 *
...
class Nice_Infoboxes_Template_Handler {
	...
	public static function get_template_part( $slug, $name = null, $load = true ) {
		...
		do_action( 'nice_infoboxes_get_template_part_' . $slug, $slug, $name );
		...
		$templates = apply_filters( 'nice_infoboxes_get_template_part', $templates, $slug, $name );

		$located_template = self::locate_template( $templates, $load, false );
		$located_template = apply_filters( 'nice_infoboxes_located_template', $located_template );
	...
	public static function get_theme_template_paths() {
		...
		$file_paths = apply_filters( 'nice_infoboxes_template_paths', $file_paths );
	...
	public static function get_templates_dir() {
		// Allow by-pass.
		if ( $templates_path = apply_filters( 'nice_infoboxes_get_templates_dir', '' ) ) {
			return trailingslashit( $templates_path );
		}

		if ( defined( 'NICE_INFOBOXES_TEMPLATES_PATH' ) && NICE_INFOBOXES_TEMPLATES_PATH ) {
			return trailingslashit( NICE_INFOBOXES_TEMPLATES_PATH );
		}
	 ...
	 *
	 * Themes can filter this by using the nice_infoboxes_templates_dir filter.
	 *
	 * @since  1.0
	 *
	 * @return string
	...
	public static function get_theme_template_dir_name() {
		$templates_dir = apply_filters( 'nice_infoboxes_templates_dir', nice_infoboxes_textdomain() );
