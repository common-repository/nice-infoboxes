...
/**
 * Class Nice_Infoboxes_PluginService
 *
 * This class deals with typical operations over Nice_Infoboxes_Plugin instances.
 *
 * @package Nice_Infoboxes_Plugin_API
 * @author  NiceThemes <hello@nicethemes.com>
 * @since   1.0
 */
class Nice_Infoboxes_PluginService extends Nice_Infoboxes_Service {
	/**
	...
	protected static function get_init_data() {
		...
		$data['plugin_file']            = nice_infoboxes_plugin_file( true );
		$data['plugin_domain']          = nice_infoboxes_plugin_domain( true );
		...
		$data['using_nicetheme']        = nice_infoboxes_is_nicetheme( $data['theme_template'] );
		$data['settings_name']          = 'nice_infoboxes_settings';
	 ...
	 * @since 1.0
	 *
	 * @param Nice_Infoboxes_Plugin|null $plugin
	 */
	public static function refresh_settings( $plugin = null ) {
		if ( ! $plugin instanceof Nice_Infoboxes_Plugin ) {
		 ...
		 */
		nice_infoboxes_update( $plugin, array(
	 ...
	 */
	public static function set_supported_integrations( Nice_Infoboxes_Plugin $plugin ) {
		/**
		 * Update plugin instance.
		 */
		nice_infoboxes_update( $plugin, array(
	 ...
	 *
	 * @uses  nice_infoboxes_integration_create()
	 *
	 * @since 1.0
	 *
	 * @return array
	...
	protected static function get_supported_integrations() {
		/**
		 * @hook nice_infoboxes_supported_integrations
		 *
		 ...
		 * @since 1.0
		 */
		$integrations = apply_filters( 'nice_infoboxes_supported_integrations', array() );
		...
		if ( ! empty( $integrations ) ) {
			foreach ( $integrations as $data ) {
				/**
				 * Create instances of `Nice_Infoboxes_Integration` with the
				 * received data and add them to our collection.
				 *
				 * @see Nice_Infoboxes_Integration
				 */
				$supported_integrations[] = nice_infoboxes_integration_create( $data );
			}
		}
	 ...
	 * @param Nice_Infoboxes_Plugin $plugin
	 */
	public static function set_i18n( Nice_Infoboxes_Plugin $plugin ) {
		/**
		 * Update plugin instance.
		 */
		nice_infoboxes_update( $plugin, array(
	...
	protected static function get_i18n() {
		 ...
		 */
		return nice_infoboxes_i18n_create( apply_filters( 'nice_infoboxes_plugin_i18n_data', array(
	...
	protected static function get_post_types() {
		$post_types_data = apply_filters( 'nice_infoboxes_post_types', array() );
		$post_types      = array();

		if ( ! empty( $post_types_data ) ) {
			foreach ( $post_types_data as $data ) {
				$post_types[] = nice_infoboxes_post_type_create( $data );
			}
		}
